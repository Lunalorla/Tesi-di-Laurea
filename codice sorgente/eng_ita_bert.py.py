...
def target_extraction(filename1):
  ...
  with open(filename1, 'r', encoding = "utf-8") as f:
    for line in f:
        ...
        ita_word = line[0]
        eng_targets.append(eng_word)
        ita_targets.append(ita_word)

  # with open(filename2, 'w', encoding = 'utf-8') as fp:
  #   for word in ita_targets:
  #     fp.write('%s\n' % (word))
...
nlp_fill = pipeline('fill-mask', model='xlm-roberta-base')

# targets = ['apple']
# #qui devi mettere tutte le parole target

# sentences = {'apple': [['I am eating a beautiful','apple',' that your mom bought.']]}
# #qui devi salvare left_context, target e right_context

#ESTRAZIONE PAROLE TARGET PER CLASSE SEMANTICA
targets = target_extraction('noun.txt')

#CREAZIONE DIZIONARIO DI PAROLE TARGET CON FRASI
sentences = create_sentences('wic_en_nomi.txt.gz', targets)
...
for i,target in enumerate(targets):
  ...
  with open('File_elaborati/Eng/Token_Nomi/mapping_dict.txt', 'wb+') as f:
        pickle.dump(mapp,f)

  vecs_token = np.array(vecs_token)
  np.save('File_elaborati/Eng/Token_Nomi/Vecs_Token/vecs_token_%s.npy'%target,vecs_token)

  vecs_cls = np.array(vecs_cls)
  np.save('File_elaborati/Eng/Token_Nomi/Vecs_cls/vecs_cls_%s.npy'%target,vecs_cls)
